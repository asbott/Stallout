...
enum File_Dialog_Flags
{
    FILE_DIALOG_FLAGS_NONE = 0,
    FILE_DIALOG_FLAGS_FILE = BIT(1),
    FILE_DIALOG_FLAGS_DIRECTORY = BIT(2),
    FILE_DIALOG_FLAGS_ALLOW_MULTIPLE = BIT(3),
    FILE_DIALOG_FLAGS_MODAL = BIT(4),
    FILE_DIALOG_FLAGS_CREATE_FILE = BIT(5),
    FILE_DIALOG_FLAGS_ALLOW_CREATE_DIRECTORY = BIT(6),
};
FLAGIFY(File_Dialog_Flags);

enum Message_Box_Flags
{
    MESSAGE_BOX_FLAGS_NONE = 0,
    MESSAGE_BOX_FLAGS_YESNO = BIT(1),
    MESSAGE_BOX_FLAGS_OKCANCEL = BIT(2),
    MESSAGE_BOX_FLAGS_OK = BIT(3),
};
FLAGIFY(Message_Box_Flags);

using namespace graphics;

ST_API void init(Graphics_Driver* driver, os::Window* window);

ST_API void set_renderer(ImGui_Renderer* rend);
ST_API ImGui_Renderer* get_renderer();
...
ST_API bool color_input(const char* name, mz::color& value);
...
ST_API bool begin_menu(const char* name, bool enabled = true);
ST_API void end_menu();

ST_API void text(const char* fmt, ...);
ST_API void colored_text(mz::color color, const char* fmt, ...);
...
ST_API mz::fvec2 get_item_pos();
ST_API mz::fvec2 get_item_size();

ST_API bool begin_menu_bar();
ST_API void end_menu_bar();

ST_API bool begin_main_menu_bar();
ST_API void end_main_menu_bar();

ST_API bool push_treenode(const char* name);
ST_API void pop_treenode();
...
ST_API void push_id(const char* id);
ST_API void push_id(const int id);
ST_API void pop_id();

// Filter in format "png,bmp,txt"
ST_API void open_file_dialog(const char* title, const std::function<void(Array<String>&)>& callback, const char* filter = 0, File_Dialog_Flags flags = FILE_DIALOG_FLAGS_NONE);
ST_API void close_file_dialog();
ST_API bool show_file_dialog();

ST_API void open_message_box(const char* message, const std::function<void(bool)>& callback = 0, Message_Box_Flags flags = MESSAGE_BOX_FLAGS_NONE);
ST_API bool show_message_box();

ST_API void sameline(f32 offset = 0.f, f32 spacing = -1.f);
ST_API void indent(f32 width = 0.f);
ST_API void align_right(f32 item_width);
ST_API void align_center(f32 item_width);
ST_API void spacing();
ST_API void separator();

ST_API mz::fvec2 measure_string(const char* str);
ST_API mz::fvec2 measure_button(const char* label);
