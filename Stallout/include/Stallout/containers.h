...
struct Forbidden_Array {
	...
	constexpr struct iterator {
        ...
        constexpr iterator& operator+=(const std::ptrdiff_t& movement) {
            _p += movement;
            return *this;
        }
        ...
        constexpr iterator operator+(const std::ptrdiff_t& movement) const {
            return iterator(_p + movement);
        }
        ...
        constexpr std::ptrdiff_t operator-(const iterator& other) const {
            return _p - other._p;
        }

        constexpr type_t& operator*() const {
            return *_p;
        }
...
using Hash_Map = std::unordered_map<T, U, std::hash<T>, GeneralEqual<T>, Global_Allocator::STL_Global_Allocator<std::pair<const T, U>>>;

template <typename T, typename U>
using Ordered_Map = std::map<T, U, GeneralLess<T>, Global_Allocator::STL_Global_Allocator<std::pair<const T, U>>>;

template <typename T>
using Hash_Set = std::unordered_set<T, std::hash<T>, GeneralEqual<T>, Global_Allocator::STL_Global_Allocator<T>>;
...
template <typename T>
using List = std::list<T, Global_Allocator::STL_Global_Allocator<T>>;
...
template <typename value_t, typename storage_container_t = Array<value_t>>
struct Static_Array2D {

    storage_container_t data;
    const size_t nrows, ncols;

    explicit Static_Array2D(size_t ncols, size_t nrows) : nrows(nrows), ncols(ncols), data(ncols * nrows) {
        
    }

    size_t to_index(size_t xpos, size_t ypos) const {
        return ncols * ypos + xpos;
    }
    void to_pos(size_t index, size_t* x, size_t* y) const {
        if (x) *x = index % ncols;
        if (y) *y = index / ncols;
    }
    ...
    bool contains(size_t xpos, size_t ypos) const {
        return contains(to_index(xpos, ypos));
    }
    ...
    value_t& front() { return data.front(); }
    value_t& back() { return data.back(); }
    
    value_t& get(size_t index) {
        return data[index];
    }
    value_t& get(size_t xpos, size_t ypos) {
        return data[to_index(xpos, ypos)];
    }
    const value_t& get(size_t index) const {
        return data[index];
    }
    const value_t& get(size_t xpos, size_t ypos) const{
        return data[to_index(xpos, ypos)];
    }
    ...
    const value_t& operator ()(size_t xpos, size_t ypos) const {
        return data[to_index(xpos, ypos)];
    }
    ...
    value_t& operator ()(size_t xpos, size_t ypos) {
        return data[to_index(xpos, ypos)];
    }
};
