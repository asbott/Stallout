...
void Double_Buffered_Thread::send(void* command, const void* data, size_t data_size) {
    ...
    // TODO (2023-12-04): #waste #memory #overhead #design #fix
    // No need to allocated data if empty. Maybe just an empty
    // flag in command? Have to pass some data because
    // traverse_commands iterates data advancing by data_size.

    if (_writebuffer_allocator._next + total_size >= _writebuffer_allocator._tail) {
        log_warn("Render command buffer overflow; swapped early");
        this->swap();
    }

    ST_ASSERT(total_size < (size_t)(_writebuffer_allocator._tail - _writebuffer_allocator._head), "Command too large");
...
void Double_Buffered_Thread::sync() {
    std::unique_lock lock(_read_mutex);
    _swap_condition.wait(lock, [&]() { 
        _read_condition.notify_one();
        return _buffer_state == COMMAND_BUFFER_STATE_OLD; 
    });
}
