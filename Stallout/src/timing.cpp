...
void FPS_Profiler::reset() {
    num_frames = 0;
    sum_frametime = 0;
    low_frametime = FLT_MAX;
    high_frametime = 0;
}
void FPS_Profiler::add_frame(f32 duration) {
    num_frames++;
    sum_frametime += duration;

    if (duration < low_frametime) low_frametime = duration;
    if (duration > high_frametime) high_frametime = duration;
}
